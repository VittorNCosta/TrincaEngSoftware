...
const { LEVELS, LEVEL_BY_ID, createTileGroups } = require('../src/data/levels.ts');
const { MATERIAL_TYPES } = require('../src/domain/recycling/value-objects/MaterialType.ts');
...
test('createTileGroups nao quebra com kindCount <= 0', () => {
  [0, -1, -4, -5, -100].forEach((kindCount) => {
    ...
    assert.ok(groups.length > 0, `kindCount=${kindCount} produziu grupos vazios`);
    assert.equal(groups.length, 1, `kindCount=${kindCount} deveria cair para 1 material`);
    assert.equal(groups[0][0], MATERIAL_TYPES[0]);
    assert.equal(groups.reduce((total, [, count]) => total + count, 0), 9);
  });
});
...
test('createTileGroups sempre devolve ciclos fechados e o total pedido de pecas', () => {
  [9, 12, 21, 45, 60].forEach((tileCount) => {
    [-2, 0, 1, 2, 3, 5, 8, 99].forEach((kindCount) => {
      ...
      assert.equal(groups.reduce((total, [, count]) => total + count, 0), tileCount);
      groups.forEach(([, count]) => assert.equal(count % 3, 0, 'contagem tem de ser multipla de 3'));
    });
  });
});
...
test('o clamp nao altera a saida para os kindCount reais (3..8)', () => {
  const GOLDEN_PRE_CLAMP = {
    9: {
      3: [['plastico', 3], ['papel', 3], ['vidro', 3]],
      4: [['plastico', 3], ['papel', 3], ['vidro', 3]],
      5: [['plastico', 3], ['papel', 3], ['vidro', 3]],
      6: [['plastico', 3], ['papel', 3], ['vidro', 3]],
      7: [['plastico', 3], ['papel', 3], ['vidro', 3]],
      8: [['plastico', 3], ['papel', 3], ['vidro', 3]],
    },
    21: {
      3: [['plastico', 9], ['papel', 6], ['vidro', 6]],
      4: [['plastico', 6], ['papel', 6], ['vidro', 6], ['metal', 3]],
      5: [['plastico', 6], ['papel', 6], ['vidro', 3], ['metal', 3], ['organico', 3]],
      6: [['plastico', 6], ['papel', 6], ['vidro', 3], ['metal', 3], ['organico', 3]],
      7: [['plastico', 6], ['papel', 6], ['vidro', 3], ['metal', 3], ['organico', 3]],
      8: [['plastico', 6], ['papel', 6], ['vidro', 3], ['metal', 3], ['organico', 3]],
    },
    60: {
      3: [['plastico', 21], ['papel', 21], ['vidro', 18]],
      4: [['plastico', 15], ['papel', 15], ['vidro', 15], ['metal', 15]],
      5: [['plastico', 12], ['papel', 12], ['vidro', 12], ['metal', 12], ['organico', 12]],
      6: [['plastico', 12], ['papel', 12], ['vidro', 12], ['metal', 12], ['organico', 12]],
      7: [['plastico', 12], ['papel', 12], ['vidro', 12], ['metal', 12], ['organico', 12]],
      8: [['plastico', 12], ['papel', 12], ['vidro', 12], ['metal', 12], ['organico', 12]],
    },
  };
...
test('getLevelsForWorld memoizado devolve as fases certas de cada mundo', () => {
  WORLDS.forEach((world) => {
    ...
    assert.equal(levels.length, world.levelIds.length, `mundo ${world.id}: quantidade`);
    assert.deepEqual(levels.map((level) => level.id), world.levelIds, `mundo ${world.id}: ordem e ids`);
    assert.ok(levels.every((level) => level.worldId === world.id), `mundo ${world.id}: worldId`);
  });
});
...
test('getLevelsForWorld e estavel entre chamadas e cobre as 103 fases', () => {
  WORLDS.forEach((world) => {
    ...
    assert.equal(JSON.stringify(second), JSON.stringify(first), `mundo ${world.id}: memo divergiu`);
  });
