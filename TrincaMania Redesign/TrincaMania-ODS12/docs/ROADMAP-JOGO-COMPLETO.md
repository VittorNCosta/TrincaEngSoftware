...
| ~~F0-03~~ | ~~Fixar versão de Node do projeto~~             | [CC]   | P1   | ✅ **Feito 2026-09-03** (`c8311ab`). `.nvmrc` com `20` + `engines: node >=20.19.0`. Foi o `engines` que revelou o bug do CI-06: se a mínima é a 20, `npm test` tinha de funcionar da 20 em diante — e não funcionava da 22. |
...
| C-25 | Escrever migração de save 203→100                                     | [CC]          | P0   | **Crítico.** Saves existentes têm ids `w1-011`…`w1-025`, `w2-*`…`w8-*` que deixam de existir. `normalizeProgress` descarta id desconhecido _em silêncio_ (invariante #3) — jogador perderia progresso sem aviso. Decidir: mapear proporcionalmente, ou versionar o save e resetar com aviso. |
...
| ~~CI-05~~  | ~~Fixar actions por SHA, não por tag~~                         | [CC] | P1   | ✅ **Feito 2026-09-03.** `checkout` em `11d5960`, `setup-node` em `49933ea`, ambos v4.4.0, com a versão no comentário ao lado.                                                                                                                                                                                                                    |
...
| SEC-04     | `dependabot.yml` para npm + github-actions                     | [CC]   | P1   | Atualiza dependência _e_ versão de action                                                                                                                                                       |
